...
This repository is the official implementation of a frozen GPT-2 transformer backbone paired with an environment-aware, scene-conditioned hypernetwork that generates Low-Rank Adaptation (LoRA) weights for physical-layer wireless communications. All models and baselines are evaluated on the 3GPP TR 38.901 Urban Macro (`Urban_Macro`) propagation environment across a standard SNR grid from $-5$ to $25$ dB.
